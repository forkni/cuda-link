...
_DTYPE_SIZES: dict[str, int] = {
    "float32": 4, "float16": 2, "bfloat16": 2, "uint8": 1, "uint16": 2, "int8": 1, "int16": 2,
}
    ...
    def from_shm(cls, shm_buf: object, num_slots: int) -> Format | None:
        ...
        try:
            ...
            if width > 0 and height > 0 and num_comps > 0:
                ...
                return cls(
                    width=width, height=height, num_comps=num_comps,
                    kind=kind, bits=bits, flags=flags,
                    dtype_str=dtype_str, shape=shape,
                    numpy_dtype=numpy_dtype, frame_nbytes=frame_nbytes,
                )
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    ...
                    "(expect ~2x slower D2H bandwidth): %s", e2,
                )
...
class Importer:
    ...
    def _try_acquire(self) -> tuple[AcquireResult | None, ImportOutcome]:
        ...
        if result.state is SlotState.VERSION_CHANGED:
            logger.debug(
                ...
                self._conn.ipc_version, result.new_version,
            )
            self._reinitialize()
    ...
    def get_frame(self, stream: object | None = None) -> ImportResult:
        ...
        self.last_latency = (
            (time.perf_counter() - producer_timestamp) * 1000
            if producer_timestamp > 0 else 0.0
        )

        conn = self._conn
        wait_start = time.perf_counter() if debug else 0.0
    ...
    def _reinitialize(self) -> None:
        ...
        try:
            ...
            if width > 0 and height > 0 and num_comps > 0:
                ...
                new_fmt = Format(
                    width=width, height=height, num_comps=num_comps,
                    kind=kind, bits=bits, flags=flags,
                    dtype_str=new_dtype_str, shape=new_shape,
        ...
        if new_fmt != self._format:
            logger.info(
                ...
                self._format.shape, self._format.dtype_str,
                new_fmt.shape, new_fmt.dtype_str,
            )
            if self._numpy is not None:
                self._numpy.close()
                self._numpy = None
    ...
    def _resolve_stream(self, stream: object) -> int:
        ...
        if hasattr(stream, "ptr"):
            return stream.ptr
        raise TypeError(
            f"Unsupported stream type: {type(stream)}. "
            "Expected torch.cuda.Stream, cupy.cuda.Stream, or int."
        )
    ...
    def get_stats(self) -> dict[str, object]:
        ...
        stats: dict[str, object] = {
            ...
            "tensor_device": (
                str(tensors[0].device)
                if TORCH_AVAILABLE and tensors and tensors[0] is not None
                else "N/A"
            ),
