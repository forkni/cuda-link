...
grep -qE 'git push --force( |$)' <<< "${COMMAND}" \
...
grep -qE 'git reset --hard' <<< "${COMMAND}" \
...
if grep -qE 'rm -rf' <<< "${COMMAND}" \
   && grep -qE '[.]git(/| |$)' <<< "${COMMAND}"; then
  _block 'rm -rf .git' \
