...
// Metadata::pack_into, shm_layout.hpp), and publish_frame/set_shutdown/clear_shutdown
// (the C3 ordering contract). TD-free, CUDA-free. C++20 required for std::atomic_ref
// (D4), same as ring_reader.h.
...
// restarts its frame counter, D6/R8).
//
...
// version paired with a stale/incomplete wire state (root-caused live: a receiver seeing
// VERSION_CHANGED before slot handles were rewritten tried to open handles to memory the
// producer had already freed -- exactly the bug this split fixes).
//
...
// signal, extending the C3 ordering contract (originally just write_idx, D4) to cover
// VERSION_CHANGED re-inits too. Must be called LAST in any (re)initialization sequence,
...
//
// Ordering is critical (D4's C3 ordering contract, mirroring shm_protocol.py::
// publish_frame verbatim): the consumer reads shutdown_flag BEFORE write_idx, so
// clearing shutdown_flag before the release store ensures the consumer always sees
// shutdown_flag=0 the first time it observes this write_idx. A bare release fence with a
// separate plain store (the naive C++ transcription) is formally UB -- atomic operations
// are required on both sides ([atomics.fences]); std::atomic_ref<uint32_t> is
// lock-free and address-free, valid for cross-process shared memory, and pairs with
// ring_reader's acquire load on write_idx. Callers must not replicate this sequence
// outside this function -- same rule as the Python original.
...
// (D6's teardown-ordering note: the receiver treats SHUTDOWN as close-handles-first, so
// the flag must already be visible before anything it references is freed).
