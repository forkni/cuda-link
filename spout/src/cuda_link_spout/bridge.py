        ...
        with SpoutReceiver.open(receiver_spec(args)) as rx:
            while True:
                ...
                if frame.outcome is not ReceiveOutcome.NEW_FRAME or frame.fmt is None:
                    continue
                key = (frame.width, frame.height, frame.fmt.channels, frame.fmt.dtype)
                if key != active_key:
                ...
                if outcome is FrameOutcome.FAILED:
                    ...
                    log.warning("cuda-link-spout: export() FAILED for %dx%d; reopening exporter", frame.width, frame.height)
                    exporter.close()
                    exporter = None
                    active_key = None
    finally:
