...
Step-by-step instructions for building the `CUDAIPCLink_v1.7.2.tox` component in TouchDesigner.
...
3. Save to: `TOXES\CUDAIPCLink_v1.7.2.tox` inside the project root

**Naming convention**: Use `CUDAIPCLink_v1.7.2.tox` (matches version) for clarity. The `TOXES\` subfolder keeps versioned binaries separate from source files.
...
1. Drag `CUDAIPCLink_v1.7.2.tox` from Windows Explorer into your TD network
...
| File | Location | Purpose |
|------|----------|---------|
| `CUDAIPCLink_v1.7.2.tox` | `TOXES/` | Final built .tox component |
