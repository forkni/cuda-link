...
**Line-ref refresh (2026-07-06):** all `CudaLinkOutTOP.cpp`/`CudaLinkInTOP.cpp` line references
below were re-verified against `feat/cpp-custom-top` post-Phase-1.4 (the shared-code extraction
that moved constructor/stream-setup into `cpp_top/src/common/cuda_device_session.h`). §2.1 also
notes the Out-TOP copy source is a `cudaArray`, which is why the C++ port needs the 3D-params
`cudaGraphExecMemcpyNodeSetParams` variant rather than mirroring the Python reference's 1D call.
...
|---|---|
| No CPU sync in the hot path (`CudaLinkOutTOP.cpp:658-675` comment) | BPG §9.1: "CPU-to-GPU synchronization points imply a stall in the GPU's processing pipeline and should be used sparingly." Verified: zero `cudaStreamSynchronize` / `cudaDeviceSynchronize` in `cpp_top/`. |
| Non-blocking streams, creation checked (`cuda_device_session.h:57-76`, called from `CudaLinkOutTOP.cpp:115` / `CudaLinkInTOP.cpp:86`) | Lei Mao, *CUDA Default Stream*: any blocking-stream or legacy-default-stream activity serializes device-wide — critical inside TouchDesigner's process, which issues its own CUDA/Vulkan work. No default-stream calls found in the hot path. |
...
| Stream priority treated as a hint (`cuda_device_session.h:57-76` comment) | Programming Guide: priorities cannot preempt in-flight work — they only matter at scheduling decision points. |
...
- **Why:** Out TOP submits 3 times per cook — `cudaMemcpy2DFromArrayAsync`
  (`CudaLinkOutTOP.cpp:614-617`), `cudaEventRecord` (`:621`), pass-through copy (`:646-649`), all
  inside the `CudaOpScope` block opened at `:602`.
  In TOP submits twice — `cudaStreamWaitEvent` (`CudaLinkInTOP.cpp:516-517`),
  `cudaMemcpy2DToArrayAsync` (`:521-524`), inside the block opened at `:507`. Each submission is a
  WDDM kernel-mode transition. The Python exporter's identical triplet collapsed 3→1 for −4.0 µs
  p50 (22 %) at 1080p (`docs/BENCHMARKS.md:39-90`).
- **How:**
  ...
  - The interop `arr->cudaArray` can differ every cook and the ring slot rotates ⇒ use
    `cudaGraphExecMemcpyNodeSetParams` (the full-3D-params variant — required here since the Out
    TOP's copy source is a `cudaArray`, not linear memory; the Python reference's
    `cudaGraphExecMemcpyNodeSetParams1D` only covers its own linear-to-linear D2D copy and cannot
    address array sources), or keep N per-slot `cudaGraphExec_t` instances.
...
- `bufferSize` at `CudaLinkOutTOP.cpp:312-314` is exact (e.g. 1080p RGBA uint8 = 8,294,400 B — not
  a 2 MiB multiple) and is passed straight to `cudaMalloc` (`:318`) with no rounding. A round-up in
  `reallocate()` closes a documented information-disclosure hazard between processes and
  incidentally guarantees the alignment the future vectorized kernel wants. **Keep the wire
  `Metadata.data_size` (`:414-420`) tight** — only the allocation should round; the Python exporter
  already keeps this exact split (`buffer_size` rounded for `cudaMalloc`, `data_size` tight on the
  wire).
- Confirmed: the Python exporter already rounds (`src/cuda_link/exporter.py:282-283,300`) — the
  C++ sender is the one side with the gap.
